...
void I420Render::init(int width,int height)
{
    this->width=width;
    this->height=height;

}
void I420Render::updateShow(unsigned char *buff)
{
    if(moutBuff) delete[] moutBuff;
    moutBuff = buff;
    this->update();
}
...
void I420Render::paintGL()
{
    ...
    glBindTexture(GL_TEXTURE_2D, m_idy);
    //修改纹理内容(复制内存内容)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width, height, 0, GL_RED, GL_UNSIGNED_BYTE,moutBuff);
    //与shader 关联
    ...
    glBindTexture(GL_TEXTURE_2D, m_idu);
    //修改纹理内容(复制内存内容)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width/2, height/2, 0, GL_RED, GL_UNSIGNED_BYTE, moutBuff+width*height);
    //与shader关联
    ...
    glBindTexture(GL_TEXTURE_2D, m_idv);
    //修改纹理内容(复制内存内容)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width/2, height/2, 0, GL_RED, GL_UNSIGNED_BYTE,moutBuff+width*height*5/4);
    //与shader 关联
