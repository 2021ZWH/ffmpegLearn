...
void ReadThread::run()
{
    ...
    {
        qWarning() << QString::fromLocal8Bit("打开失败！");
    }
    // 循环读取视频图像
    while (m_play)
    {
        // 暂停
         image = m_videoDecode->read();  // 读取视频图像
...
#endif
            emit updateImage(std::move(image));
        }
        else
        {
            // 当前读取到无效图像时判断是否读取完成
    ...
    qDebug() << QString::fromLocal8Bit("播放结束！");
    m_videoDecode->close();
    qDebug() << QString::fromLocal8Bit("decode close");
    emit playState(end);
    qDebug() << QString::fromLocal8Bit("发送结束信号");
}
